 ...
 * @author Manuel
 */
public class ActorRepository implements Repository<ActorDirector>{

    private static final String ID_ACTOR = "IDMovieActor";
    ...
    private static final String SELECT_PERSON_MOVIE = "{ CALL selectActorByMovie (?) }";
    private static final String DELETE_ACTORS_BY_MOVIE = "{ CALL deleteActorsByMovie (?) }";
    @Override
    public int create(ActorDirector person) throws Exception {
    ...
    @Override
    public void create(List<ActorDirector> persons) throws Exception {
        DataSource dataSource = DataSourceSingleton.getInstance();
        try(Connection con = dataSource.getConnection();
                CallableStatement stmt = con.prepareCall(CREATE_ACTOR)) {
            
            for (ActorDirector person : persons) {
                stmt.setInt(1, person.getPersonId());
                stmt.setInt(2, person.getMovieId());
    ...
    @Override
    public void update(int id, ActorDirector data) throws Exception {
        DataSource dataSource = DataSourceSingleton.getInstance();
        try(Connection con = dataSource.getConnection();
    ...
    @Override
    public Optional<ActorDirector> select(int id) throws Exception {
        DataSource dataSource = DataSourceSingleton.getInstance();
        try(Connection con = dataSource.getConnection();
            ...
            try(ResultSet rs = stmt.executeQuery()) {
                if(rs.next()) {
                    return Optional.of(new ActorDirector(
                            rs.getInt(ID_ACTOR),
                            rs.getInt(PERSON_ID),
                            rs.getInt(MOVIE_ID)));
                }
    ...
    @Override
    public List<ActorDirector> select() throws Exception {
        List<ActorDirector> persons = new ArrayList<>();
        DataSource dataSource = DataSourceSingleton.getInstance();
        try(Connection con = dataSource.getConnection();
                CallableStatement stmt = con.prepareCall(SELECT_ACTORS);
                ResultSet rs = stmt.executeQuery()) {
            while(rs.next()) {
                persons.add(new ActorDirector(
                        rs.getInt(ID_ACTOR), 
                        rs.getInt(PERSON_ID),
                        rs.getInt(MOVIE_ID)));
            }
    ...
    public static List<Person> selectByMovie(int movieId) throws Exception {
     ...
     public static void deleteByMovie(int id) throws Exception {
        DataSource dataSource = DataSourceSingleton.getInstance();
        try(Connection con = dataSource.getConnection();
                CallableStatement stmt = con.prepareCall(DELETE_ACTORS_BY_MOVIE)) {
